## Integrate the two tables with each other
#indirect = read.csv2("Indirect_connections -- Transmog 31-10-2016.csv", header = F, stringsAsFactors = F)
#indirect2 = read.csv2("Indirect_connections -- Transmog errors appendix.csv", header = F, stringsAsFactors = F)
#candidates_temp = rbind(indirect, indirect2)
#colnames(candidates_temp) = c("Connected_to_ID", "Name", "ID", "PredicateID", "SemanticTypeIDs", "nSources", "Sources")
#write.csv2(candidates_temp, "Indirect_Transmog_all 31-10-2016.csv", row.names = F)
